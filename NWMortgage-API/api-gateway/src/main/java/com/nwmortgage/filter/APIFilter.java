...
public class APIFilter implements GatewayFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (!request.getHeaders().containsKey("Authorization")) {
            ...
            byte[] responseBytes = "{\"message\":\"Authorization header is missing.\"}".getBytes(StandardCharsets.UTF_8);
        ...
        if (!authHeader.startsWith("Bearer ")) {
            response.setStatusCode(HttpStatus.UNAUTHORIZED);
            byte[] responseBytes = "{\"message\":\"Authorization header is invalid.\"}".getBytes(StandardCharsets.UTF_8);
            return response.writeWith(Mono.just(response.bufferFactory().wrap(responseBytes)));
        }
        ...
        try {
            ...
            response.setStatusCode(HttpStatus.UNAUTHORIZED);
            byte[] responseBytes = "{\"message\":\"Token is invalid.\"}".getBytes(StandardCharsets.UTF_8);
            return response.writeWith(Mono.just(response.bufferFactory().wrap(responseBytes)));
        }
